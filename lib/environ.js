			...
			if (afs.exists(mobilesdkPath)) {
				fs.readdirSync(mobilesdkPath).filter(function (f) {
					...
					var sdkPath = path.join(mobilesdkPath, v),
						...
						legacyManifest = {
							version: 'version',
							module_apiversion: 'moduleAPIVersion',
							timestamp: 'timestamp',
							githash: 'githash'
						};
					...
					if (afs.exists(manifestFile)) {
						...
						// check if there's a version.txt
						if (afs.exists(manifestFile = path.join(sdkPath, 'version.txt'))) {
							manifest = sdk.manifest = {};
							fs.readFileSync(manifestFile, 'utf-8').toString().split('\n').forEach(function (line) {
								var p = line.indexOf('=');
								if (p != -1) {
									var key = line.substring(0, p);
									manifest[legacyManifest[key] || key] = line.substring(p+1);
								}
							});
						}
					}
