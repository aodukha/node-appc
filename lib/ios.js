...
exports.detect = function (finished, opts) {
	...
	async.parallel({
		...
		iOSDevices: function (callback) {
			var bin = path.resolve(module.filename, '..', '..', 'tools', 'ios-device', 'bin', 'ios-device'),
				lib = path.resolve(module.filename, '..', '..', 'tools', 'ios-device', 'lib'),
				child = spawn(bin, ['query', '--detailed'], {
					env: {
			...
			child.on('exit', function (code) {
				...
				if (!code) {
					try {
						...
						if (Object.prototype.toString.call(obj) == '[object Object]' && obj.success && Array.isArray(obj.devices)) {
							result = obj.devices;
						}
					} catch (ex) {}
				}
		...
		finished(cached = results);
	});
};

exports.install = function (deviceId, ipaFile, logger, finished) {
	var bin = path.resolve(module.filename, '..', '..', 'tools', 'ios-device', 'bin', 'ios-device'),
		lib = path.resolve(module.filename, '..', '..', 'tools', 'ios-device', 'lib'),
		child,
		stdout = '';
	
	logger.debug(__('Installer environment: %s', ('DYLD_LIBRARY_PATH=' + lib).cyan));
	logger.debug(__('Installer command: %s', (bin + ' install ' + deviceId + ' ' + ipaFile).cyan));
	
	child = spawn(bin, ['install', deviceId, ipaFile], {
		env: {
			DYLD_LIBRARY_PATH: lib
		}
	});
	...
	child.on('exit', function (code) {
		var result = [],
			obj,
			err = code && __('ios-device failed with code: %s', code);
		
		try {
			obj = JSON.parse(stdout);
			if (Object.prototype.toString.call(obj) == '[object Object]') {
				if (code || !obj.success) {
					err = obj.error ? __(obj.error) : __('Unknown error trying to install app on device');
				}
			} else {
				err = __('ios-device output was not parsable JSON');
			}
		} catch (ex) {
			err = __('Invalid JSON returned by ios-device install command');
		}
